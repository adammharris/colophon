...
//!
//! Everything here reaches colophon only through its published surface (`use
//! colophon::…`) — no `pub(crate)` internals, no test-only backends. That is the
...
//! rollback that itself faults, [`colophon::Error::Torn`] — is reachable and
//! reported through nothing but the public [`colophon::Storage`] trait.

use std::future::Future;
use std::io;
use std::path::{Path, PathBuf};

use colophon::fs::{DirEntry, Metadata};
use colophon::{
...
fn tmp(name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("colophon-pubapi-{name}-{}", std::process::id()));
...
fn discovery_locates_a_root_through_the_public_api() {
    ...
    match block_on(colophon::discover(&StdFs, &root.join("deep"))).expect("discover") {
...
// ───────────────────────── Send-ness ────────────────────────────────────────
//
// colophon's exported *values* and its non-recursive futures must be `Send`, so
...
// a `+ Send` bound, so they are not `Send`. colophon runs them through its own
// single-threaded [`colophon::block_on`], which never required it, and adding the
...
fn public_types_are_send() {
    ...
    assert_send::<Error>();
    assert_send::<colophon::Discovered>();
    assert_send::<colophon::tree::Node>();
}
...
async fn futures_stay_send(fs: &StdFs, cs: ChangeSet) {
    ...
    require_send_future(colophon::discover(fs, Path::new(".")));
}
...
/// that fails too (leaving colophon unable to say what is on disk).
///
...
/// the undo itself faults). Op-1's atomic staging write goes to a `colophon-tmp`
...
impl DoubleFault {
    fn should_fail(&self, path: &Path) -> bool {
        ...
        // sibling, which carries a `colophon-tmp` marker in its name).
        let is_restore =
            path.file_name() == self.restore_victim.file_name() && !name.contains("colophon-tmp");
...
fn a_rollback_that_itself_fails_surfaces_as_torn() {
    ...
    // consistent applied state (colophon refuses to claim a state it cannot name).
    assert!(
        root.join(".colophon-journal").exists(),
