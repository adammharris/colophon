...
//!
//! A colophon workspace is *self-describing*, so where it begins is a fact to be
...
/// (defaults, overlaid by the root's `colophon:` block, overlaid by the linked
/// config document).
#[derive(Debug, Clone)]
...
pub enum Discovery {
    ...
    /// break the tie — colophon will not guess which is the root. Carries the
...
/// and layer the effective config (defaults → root `colophon:` block → linked
...
async fn build<FS: Storage + Clone>(
    ...
    // The root's `colophon:` frontmatter block (config's description home).
...
mod tests {
    ...
    fn tmp(name: &str) -> PathBuf {
        let dir =
            std::env::temp_dir().join(format!("colophon-discover-{name}-{}", std::process::id()));
