//! Fixity — content checksums that let colophon detect *bit-rot*, not just
//! broken links.
//!
...
//! colophon workspace's fixity is verifiable by *anyone*, with standard tools
//! (`sha256sum`, BagIt validators), not only by colophon — the same
...
//! [`crate::exec::block_on`] and the journal's FNV checksum are: colophon keeps
...
/// (a document predating fixity is not "corrupt"). A recorded value colophon
...
/// Whether `recorded` is a fixity digest colophon can actually check — the
...
mod tests {
    ...
    fn verify_never_cries_wolf_over_an_unrecorded_or_unknown_digest() {
        ...
        // A digest from an algorithm colophon does not know → unverifiable, not
