...
pub enum Error {
    ...
    /// `..`. colophon clamps every I/O to the tree it was pointed at (a link
    ...
    /// ([`crate::journal::recover`], which `colophon check` performs) first, then
    /// retry.
    #[error(
        "a previous change was interrupted and not yet recovered (found {0}); \
         recover it first (run `colophon check`), then retry"
    )]
