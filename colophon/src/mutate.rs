...
impl<FS: Storage, Id, Ix> Workspace<FS, Id, Ix> {
    ...
    pub async fn create(&self, path: &Path, parent: &Path) -> Result<()> {
    ...
    pub async fn rename(&self, from: &Path, to: &Path) -> Result<()> {
        ...
        if let Some(parent) = self.single_target(&from_doc, &inverse, &from) {
            let (parent_text, parent_doc) = self.load(&parent).await?;
            if let Some(updated) = retarget_entry(
                &parent_text,
                &parent_doc,
                &spanning,
                &parent,
        ...
        for raw in self.relations().children(&from_doc.meta) {
            ...
            let child_path = link::resolve(&from, &child.target);
    ...
    /// orphaned) unless `force` is set.
    pub async fn delete(&self, path: &Path, force: bool) -> Result<()> {
        let path = link::normalize(path);
        let (spanning, inverse) = self.spanning_pair()?;
        ...
        if let Some(parent) = self.single_target(&doc, &inverse, &path) {
            let (parent_text, parent_doc) = self.load(&parent).await?;
            let kind = parent_doc.embed.unwrap_or(EmbedType::FrontmatterYaml);
            if let Some(index) = entry_index(&parent_doc, &spanning, &parent, &path) {
    ...
    /// on-workspace path. (`doc_path` anchors the relative target.)
    fn single_target(&self, doc: &Document, field: &str, doc_path: &Path) -> Option<PathBuf> {
        ...
        Some(link::resolve(doc_path, &target.target))
    }
}

/// The index of the entry in `doc`'s `field` sequence whose target resolves to
/// `wanted` (with `doc_path` anchoring relative targets).
fn entry_index(doc: &Document, field: &str, doc_path: &Path, wanted: &Path) -> Option<usize> {
    doc.meta
        .get(field)
        .map(Value::link_strings)?
        .iter()
        .position(|raw| link::resolve(doc_path, &Link::parse(raw).target) == *wanted)
}

/// Rewrite the entry of `field` in `doc` whose target resolves to `old` so it
/// reaches `new` instead, preserving the entry's label and the document's
/// formatting. Returns the updated text, or `None` when no entry matches.
fn retarget_entry(
    text: &str,
    doc: &Document,
    field: &str,
    ...
    let Some(index) = entries
        .iter()
        .position(|raw| link::resolve(doc_path, &Link::parse(raw).target) == *old)
    else {
        return Ok(None);
    };
    let updated = Link::parse(&entries[index]).with_target(link::relative(dir, new));
    ...
    match (cardinality, is_seq) {
        (_, true) => embed.replace_value(
            &[Segment::Key(field), Segment::Index(index)],
            fig::Value::Str(updated.render()),
        )?,
        (Cardinality::One, false) | (Cardinality::Many, false) => {
...
/// document moves from `from` to `to`. External targets are untouched.
fn rerelativize(
    text: &str,
    doc: &Document,
