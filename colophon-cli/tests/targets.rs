...
fn journal(tag: &str) -> std::path::PathBuf {
    let dir = std::env::temp_dir().join(format!("colophon-targets-{tag}-{}", std::process::id()));
...
fn a_bare_path_needs_no_workspace() {
    ...
    // must not smuggle in.
    let dir = std::env::temp_dir().join(format!("colophon-targets-loose-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
fn a_route_outside_a_workspace_says_so_rather_than_reading_a_file() {
    let dir = std::env::temp_dir().join(format!("colophon-targets-noroot-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
